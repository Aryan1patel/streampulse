...
def predict_lstm(price_history: List[float], features: Dict[str, Any]) -> List[float]:
    """
    ...
    try:
        ...
        if len(price_history) < seq_len:
            ...
            logger.warning(f"Short history for LSTM: {len(price_history)} < {seq_len}. Using default 10-day fallback.")
            seq_len = 10
        ...
        sentiment_bias = features.get('sentiment_impact', 0) * 0.5
        ...
        for day in range(3):
            ...
            scaled_move = np.clip(pred_raw * 2.0, -2.5, 2.5)
            day_move = scaled_move + sentiment_bias
            
            # Update cumulative trajectory
            # (1 + total) = (1 + prev_total) * (1 + day_move/100)
        ...
        logger.info(f"LSTM 3-Day Forecast: {predictions}")
        return predictions
